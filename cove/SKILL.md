---
name: cove
description: Chain-of-Verification (CoVe) prompting system. Converts lazy prompts into rigorous 4-stage verified output. Use for any code generation, debugging, or implementation task. Reduces hallucinations and catches subtle bugs.
disable-model-invocation: false
user-invocable: true
argument-hint: "[your task or lazy prompt]"
---

**IMPORTANT: Never auto-commit. Always present the final output for user review before committing.**
...
- **Standalone**: `$cove [task]` — runs the 4-stage verification protocol directly
- **Via `$dispatch`**: `$dispatch` automatically routes non-trivial code to `$cove` after equipping skills and expanding the prompt
- **Via `$tdd`**: During the GREEN phase, `$tdd` invokes `$cove` for non-trivial implementations (stateful, async, auth, database)

Use `$cove` directly when you just want verified code without the full dispatch pipeline.
